...
var getNodeHandlers = function(node) {
	return {
		type: function() {
			var firstChar = node.charAt(0);
			if (firstChar == "\"") return "literal";
		...
		dataType: function() {
			if (node.charAt(0) == "\"") {
				var typeSpecifier = node.lastIndexOf("^", node.length - node.lastIndexOf("\""));
				if (typeSpecifier > 0) return node.slice(typeSpecifier + 2, -1);//this removes the < and > from datatype as well
			}
			return null;
		},
		lang: function() {
			if (node.charAt(0) == "\"") {
				var langSpecifier = node.lastIndexOf("@", node.lastIndexOf("\""));
				if (langSpecifier > 0) return node.slice(langSpecifier + 1);
			}
			return null;
		}
...
if (process.argv.length <= 2) {
	console.log("Please pass the gzip data source as argument")
}
